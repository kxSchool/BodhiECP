...
class Membersmodel extends CI_Model {
    ...
    //查询账户余额
    public function getShopMoney($userid)
    {

        //首先根据会员的id，判定需要查询哪个会员表
        $maxNUmber = $this->config->item('max_members');//每张用户表最大存储记录数
        $tableName = 'members'.ceil($userid/$maxNUmber);
        $this->slave->select('money');
        $this->slave->where('userid',$userid);
        $this->slave->from($tableName);
        $query = $this->slave->get();
        $result = $query->row_array();
    ...
    //修改账户余额
    public function saveShopMoney($userid,$new_money)
    {
        ...
        $tableName = 'members'.ceil($userid/$maxNUmber);
        $this->master->where('userid',$userid);
        $result = $this->master->update($tableName,array('money'=>$new_money));
        return $result;
    }
    ...
    public function getAllAgency()
    {
        ...
        if(!empty($memberTables)){
            foreach($memberTables as $v){
                ...
                $this->slave->from($tablename);
                $this->slave->where(array('type'=>2));
                $this->slave->select('userid,username');
                $query = $this->slave->get();
                $result = $query->result_array();
                $totalResult = array_merge($result,$totalResult);
            }
    ...
    public function getAgencyInfo($data)
    {
        ...
        $maxNUmber = $this->config->item('max_members');//每张用户表最大存储记录数
        $tableName = 'members'.ceil($userid/$maxNUmber);
        $this->slave->where(array('userid'=>$userid));
        $this->slave->from($tableName);
        $query = $this->slave->get();
        $result = $query->row_array();
    ...
    public function getAgencyList($data)
    {
        ...
        if(!empty($memberTables)){
            foreach($memberTables as $v){
                ...
                $query = $this->slave->get();
                $result['total'] += $query->num_rows();
                $result['rows'] = array_merge($query->result_array(),$result['rows']);
            }
        }
        return !empty($result) ? $result : '';
    }
    ...
    public function userCount($where=array())
    {
        ...
        if(!empty($memberTables)){
            foreach($memberTables as $v){
                $tablename = substr($v['TABLE_NAME'],strlen($this->slave->dbprefix));;//去除表前缀
                $this->slave->where($where);
                $count = $this->slave->count_all_results($tablename);
                $total += $count;
            }
        }
        return $total;
