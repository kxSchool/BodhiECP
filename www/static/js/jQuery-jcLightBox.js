		...
		return this.each(function(i, t) {
			...
			$RelList.hover(function() {
				...
				if($thisA.data("value") == "rel") {
					$thisA.find("span").fadeTo(options.listSpeed, .6).next().animate({
						"bottom": nTextBottom,
						"opacity": 1
					}, options.listSpeed);
					return false;
				...
				var $thisA = $(this);
				if($thisA.data("value") == "rel") {
					$thisA.find("b").animate({
						"bottom": 0,
						"opacity": 0
					}, options.listSpeed).prev().fadeTo(options.listSpeed, 0);
					return false;
				};
			...
			function fadeState(nw, nh) {
				_w = nw, _h = nh, _l = (nWinWidth - _w) / 2, _t = (nWinHeight - _h) / 2;
				return [_t, _l, _w, _h];
			};
